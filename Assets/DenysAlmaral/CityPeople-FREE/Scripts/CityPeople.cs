...
namespace CityPeople
{
    public class CityPeople : MonoBehaviour
    {
        ...
        [SerializeField] [Tooltip("Autoplay random animation clips")]
        private bool AutoPlayAnimations = true;

        [SerializeField] [Tooltip("Overrides palette materials, skips other objects")]
        ...
        private void Awake()
        {
            ...
            foreach (var r in AllRenderers)
            {
                ...
                if (matName[..len] == people_pal_prefix) _paletteMeshes.Add(r);
            }

            if (_paletteMeshes.Count > 0) CurrentPaletteName = _paletteMeshes[0].sharedMaterial.name;

            if (PaletteOverride != null) SetPalette(PaletteOverride);
        }
        ...
        public void SetPalette(Material mat)
        {
            if (mat != null)
            {
                if (mat.name[..people_pal_prefix.Length] == people_pal_prefix)
                {
                    CurrentPaletteName = mat.name;
                    foreach (var r in _paletteMeshes) r.material = mat;
                }
                else
                {
